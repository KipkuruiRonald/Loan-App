...
def _get_default_system_settings():
    ...
    return {
        "general": [
            {"key": "site_name", "value": "Okolea", "type": "string", "description": "Platform name", "is_editable": True},
            {"key": "support_email", "value": "support@okolea.com", "type": "string", "description": "Support email", "is_editable": True},
...
# ============================================================================
# PUBLIC LOAN PARAMETERS ENDPOINT (No Auth Required)
# ============================================================================

@router.get("/loan-parameters")
async def get_loan_parameters(
    db: Session = Depends(get_db)
):
    """Get current loan parameters for all users (no auth required)"""
    ...
    for setting in settings:
        ...
        params[setting.setting_key] = value
    
    # Return with defaults if not set
    return {
        "default_interest_rate": float(params.get("default_interest_rate", 4.0)),
        "penalty_rate": float(params.get("penalty_rate", 6.8)),
        "term_days": int(params.get("term_days", 9)),
        "min_loan_amount": float(params.get("min_loan_amount", 500)),
        "max_loan_amount": float(params.get("max_loan_amount", 15000)),
        "processing_fee": float(params.get("processing_fee", 0)),
        "perfect_repayment_bonus": int(params.get("perfect_repayment_bonus", 40)),
    }
...
async def export_user_data(
    ...
    pdf.cell(0, 10, 'Okolea - User Data Export', 0, 1, 'C')
    pdf.ln(5)
    ...
    pdf.cell(0, 5, 'Generated by Okolea Loan App', 0, 1, 'C')
